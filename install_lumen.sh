...
# Corregido: usa el hostname estático de la Pi para obtener una asignación ÚNICA
#            (evita que todas las cajas pidan "Box" y se reciclen/dupliquen)
#
# - Crea/usa clave SSH local
# - Autoriza Pi -> VPS (ssh-copy-id)  [pedirá la contraseña del VPS solo 1 vez]
# - Pide/recicla DEVICE_ID y PORT en el VPS (lumen-assign.sh <hostname_unico>)
...
if [[ -n "${DEVICE_ID}" && -n "${PORT}" ]]; then
  ...
  # Usar hostname ESTÁTICO de la Pi como identificador único
  ...
  if echo "$ASSIGN_RAW" | grep -q 'DEVICE_ID='; then
    ...
    DEVICE_ID="$(echo "$ASSIGN_RAW" | jq -r '.device_id // empty' 2>/dev/null || true)"
    PORT="$(echo "$ASSIGN_RAW" | jq -r '.port // empty' 2>/dev/null || true)"
  fi
